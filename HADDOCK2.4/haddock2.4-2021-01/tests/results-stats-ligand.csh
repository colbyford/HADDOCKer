...
foreach i ($argv)
  ...
  if (-e $clufi) then
    awk '{if ($6-$7<=2) {print "  ",NR-1,$1,$6,$7,$2,$3}}' $clufi | sed -e 's/file\.nam_//'| grep -v "#" >ttmp1
    cat /dev/null >ttmp2
  ...
  grep $target $rmsfile | awk '{if ($2<=1) {print NR,$1,$2}}' | head -1 >ttmp1
  if (`grep $target ttmp1 | wc -l |awk '{print $1}'` > 0) then
    grep `awk '{print $2}' ttmp1` $fnatfile >ttmp2
    paste ttmp1 ttmp2 |awk '{printf  "water first medium <1A          : Rank %4d i-RMSD: %5.2f  Fnat: %5.2f\n",$1,$3,$5}'
    set accept=1
  else
    echo "no water first medium <1A - - - - - - -"
  endif
  grep $target $rmsfile | awk '{if ($2<=2) {print NR,$1,$2}}' | head -1 >ttmp1
  if (`grep $target ttmp1 | wc -l |awk '{print $1}'` > 0) then
    grep `awk '{print $2}' ttmp1` $fnatfile >ttmp2
    paste ttmp1 ttmp2 |awk '{printf  "water first acceptable <2A      : Rank %4d i-RMSD: %5.2f  Fnat: %5.2f\n",$1,$3,$5}'
    set accept=1
  else
    echo "no water first acceptable <2A - - - - - - -"
  endif
  grep $target $rmsfile | awk '{if ($2<=2) {print NR,$1,$2}}' | head -1 >ttmp1
  if (`grep $target ttmp1 | wc -l |awk '{print $1}'` > 0) then
    grep $target $rmsbestfile | head -1 | awk '{if ($2<=2) {print $1}}' | head -1 >ttmp0
  ...
  if (`grep $target ttmp1 | wc -l |awk '{print $1}'` > 0) then
    grep `awk '{print $2}' ttmp1` $fnatfile >ttmp2
    paste ttmp1 ttmp2 |awk '{printf  "it1 first medium <1A            : Rank %4d i-RMSD: %5.2f  Fnat: %5.2f\n",$1,$3,$5}'
    set accept=1
  else
    echo "no it1 acceptable <1A - - - - - - -"
  endif
  grep $target $rmsfile | awk '{if ($2<=2) {print NR,$1,$2}}' | head -1 >ttmp1
  if (`grep $target ttmp1 | wc -l |awk '{print $1}'` > 0) then
    grep `awk '{print $2}' ttmp1` $fnatfile >ttmp2
    paste ttmp1 ttmp2 |awk '{printf  "it1 first acceptable <2A        : Rank %4d i-RMSD: %5.2f  Fnat: %5.2f\n",$1,$3,$5}'
    set accept=1
  else
    echo "no it1 first acceptable <2A - - - - - - -"
  endif
  grep $target $rmsfile | awk '{if ($2<=2) {print NR,$1,$2}}' | head -1 >ttmp1
  if (`grep $target ttmp1 | wc -l |awk '{print $1}'` > 0) then
    grep $target $rmsbestfile | head -1 | awk '{if ($2<=2) {print $1}}' | head -1 >ttmp0
  ...
  if (`grep $target ttmp1 | wc -l |awk '{print $1}'` > 0) then
    grep `awk '{print $2}' ttmp1` $fnatfile >ttmp2
    paste ttmp1 ttmp2 |awk '{printf  "it0 first medium <1A            : Rank %4d i-RMSD: %5.2f  Fnat: %5.2f\n",$1,$3,$5}'
    set accept=1
  else
    echo "no it0 acceptable <1A - - - - - - -"
  endif
  grep $target $rmsfile | awk '{if ($2<=2) {print NR,$1,$2}}' | head -1 >ttmp1
  if (`grep $target ttmp1 | wc -l |awk '{print $1}'` > 0) then
    grep `awk '{print $2}' ttmp1` $fnatfile >ttmp2
    paste ttmp1 ttmp2 |awk '{printf  "it0 first acceptable <2A        : Rank %4d i-RMSD: %5.2f  Fnat: %5.2f\n",$1,$3,$5}'
    set accept=1
  else
    echo "no it0 first acceptable <2A - - - - - - -"
  endif
  grep $target $rmsfile | awk '{if ($2<=2) {print NR,$1,$2}}' | head -1 >ttmp1
  if (`grep $target ttmp1 | wc -l |awk '{print $1}'` > 0) then
    grep $target $rmsbestfile | head -1 | awk '{if ($2<=2) {print $1}}' | head -1 >ttmp0
  ...
  grep pdb $i/structures/it0/i-RMSD.dat | awk '$2<=2' | wc -l
  ...
  echo 'it0: structures within best200 with i-RMSD<0.5A:' |awk '{printf "%s   %s %s %s %s %s ",$1,$2,$3,$4,$5,$6}'
  grep pdb $i/structures/it0/i-RMSD.dat | head -200 | awk '$2<=0.5' | wc -l
  ...
  echo 'it1: structures with i-RMSD<0.5A:' |awk '{printf "%s   %s %s %s %s ",$1,$2,$3,$4,$5}'
  grep pdb $i/structures/it1/i-RMSD.dat | awk '$2<=0.5' | wc -l
  ...
  echo 'water: structures with i-RMSD<0.5A:' |awk '{printf "%s %s %s %s %s ",$1,$2,$3,$4,$5}'
  grep pdb $i/structures/it1/water/i-RMSD.dat | awk '$2<=0.5' | wc -l
end
