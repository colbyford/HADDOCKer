...
foreach iclu (file.nam_clust[0-9] file.nam_clust[0-9][0-9] file.nam_clust[0-9][0-9][0-9])
  ...
  foreach i (`cat $iclu`)
    ...
    head -50 $i | grep -i energies | sed -e's/\,//g' | awk '{print $3,$8+$9,$8+0.1*$9,$8,$9,$10,$11,$12,$13,$14,$15}'>>$iclu'_ener'
  end
...
foreach iclu (file.nam_clust[0-9] file.nam_clust[0-9][0-9] file.nam_clust[0-9][0-9][0-9])
  ...
  foreach i (`cat $iclu`)
    ...
    head -50 $i |grep -i " violation" | sed -e's/\,//g' | awk '{print $3,$4,$5,$6,$7,$8}'>>$iclu'_viol'
  end
...
foreach iclu (file.nam_clust[0-9] file.nam_clust[0-9][0-9] file.nam_clust[0-9][0-9][0-9])
  ...
  foreach i (`cat $iclu`)
    ...
    head -50 $i |grep -i buried | awk '{print $5}'>>$iclu'_bsa'
  end
...
foreach iclu (file.nam_clust[0-9] file.nam_clust[0-9][0-9] file.nam_clust[0-9][0-9][0-9])
  ...
  foreach i (`cat $iclu`)
    ...
    head -50 $i | grep -i " Binding" | awk '{print $4}'>>$iclu'_dH'
  end
...
foreach iclu (file.nam_clust[0-9] file.nam_clust[0-9][0-9] file.nam_clust[0-9][0-9][0-9])
  ...
  foreach i (`cat $iclu`)
    ...
    head -50 $i | grep -i " desolv" | awk '{print $4}'>>$iclu'_Edesolv'
  end
...
#
# if file present, calculate average cluster i-RMSD from target
#
if (-e i-RMSD.dat) then
  echo "#Cluster i-RMSD sd" >cluster_rmsd-Emin.txt
  foreach iclu (file.nam_clust[0-9] file.nam_clust[0-9][0-9])
    echo "#Structure i-RMSD" >$iclu'_rmsd-Emin'
    foreach i (`cat $iclu`)
      grep $i i-RMSD.dat >>$iclu'_rmsd-Emin'
    end
...
#
  ...
  foreach iclu (file.nam_clust[0-9] file.nam_clust[0-9][0-9] file.nam_clust[0-9][0-9][0-9])
    ...
    foreach i (`cat $iclu`)
      ...
      $PROFIT <<_Eod_ |grep RMS >>rmsd_best-Emin.disp
        refe $refe
        mobi $i
        atom $atoms
        fit
