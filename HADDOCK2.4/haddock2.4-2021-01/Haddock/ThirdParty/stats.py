...
## CHANGE LOG:
## ==========
## 01/13/98 ... CHANGED TO VERSION 0.3
...
"""
    ...
    r_den = math.sqrt((n*ss(x) - sumsquared(x))*(n*ss(y)-sumsquared(y)))
...
"""
    ...
    slope = r_num / float(n*ss(x) - sumsquared(x))
...
"""
    if len(a)<>len(b):
	raise ValueError, 'Unequal length arrays in ttest_rel.'
    x1 = mean(a)
    x2 = mean(b)
    v1 = var(a)
    v2 = var(b)
...
"""
    ...
    en2 = n2
    d = 0.0
    data1 = Numeric.sort(data1)
    data2 = Numeric.sort(data2)
	...
	if dt > d:
	    d = dt
    prob = ksprob(math.sqrt(en1*en2/float(en1+en2))*d)
    return d, prob
...
"""
    z = abs(x)
    t = 1.0 / (1.0+0.5*z)
    ans = t * math.exp(-z*z-1.26551223 + t*(1.00002368+t*(0.37409196+t*(0.9678418+t*(-0.18628806+t*(0.27886807+t*(-1.13520398+t*(1.48851587+t*(-0.82215223+t*0.17087277)))))))))
    if x >= 0:
	return ans
    else:
	return 2.0 - ans
...
"""
    ...
    bign = len(alldata)
    sstot = ass(alldata)-(asumsquared(alldata)/float(bign))
    ssbn = 0
    for list in lists:
	ssbn = ssbn + asumsquared(N.array(list))/float(len(list))
    ssbn = ssbn - (asumsquared(alldata)/float(bign))
    sswn = sstot-ssbn
    dfbn = a-1
    dfwn = bign - a
    msb = ssbn/float(dfbn)
...
def lsumsquared(inlist):
    """
Adds the values in the passed list, squares the sum, and returns
the result.

Usage:   lsumsquared(inlist)
Returns: sum(inlist[i]*inlist[i])
"""
    s = sum(inlist)
    return s*s
...
subject factor, 0=between.  Input equals the entire data array (i.e.,
...
sumsquared = Dispatch ( (lsumsquared, (ListType, TupleType)), )
...
#####################################
 ...
 def ageometricmean (inarray,dimension=None,keepdim=0):
    """
...
keepdim is set to 1, the resulting array will have as many dimensions as
inarray, with only 1 'level' per dim that was collapsed over.

Usage:   ageometricmean(inarray,dimension=None,keepdim=0)
...
"""
 ...
 def aharmonicmean (inarray,dimension=None,keepdim=0):
    """
...
Usage:   aharmonicmean(inarray,dimension=None,keepdim=0)
...
"""
 ...
 def amean (inarray,dimension=None,keepdim=0):
    """
...
Usage:   amean(inarray,dimension=None,keepdim=0)
...
be None, to pre-flatten the array, or dimension can equal 0).
...
strictly outside the list/tuple passed to 'limits'.   Note: either limit
in the list/tuple, or the value of limits itself, can be set to None.  The
inclusive sequence determines whether the lower and upper limiting bounds
(respectively) are open/exclusive (0) or closed/inclusive (1).

Usage:   atmean(a,limits=None,inclusive=(1,1))
"""
     if limits == None:
         sum = float(N.add.reduce(N.ravel(a)))
     ...
     if limits[0] > max(N.ravel(a)) or limits[1] < min(N.ravel(a)):
     ...
     sum = float(N.add.reduce(N.ravel(a*mask)))
...
ignoring values strictly outside the list/tuple passed to 'limits'.  
Note: either limit in the list/tuple, or the value of limits itself,
can be set to None.  The inclusive sequence determines whether the lower
...
"""
 ...
 def atmin(a,lowerlimit=None,dimension=None,inclusive=1):
     """
Returns the minimum value of a, along dimension, excluding any values less
than lowerlimit (or equal to, if inclusive=1).  If the limit is set to None,
a limit smaller than the min value in the array is used.
...
"""
	 ...
	 dimension = 0
     if lowerlimit == None:
	 lowerlimit = min(N.ravel(a))-11
     biggest = max(N.ravel(a))
 ...
 def atmax(a,upperlimit,dimension=None,inclusive=1):
     """
Returns the maximum value of a, along dimension, excluding any values greater
than upperlimit (or equal to, if inclusive=1).  If the limit is set to None,
...
"""
	 ...
	 dimension = 0
     if upperlimit == None:
	 upperlimit = max(N.ravel(a))+1
     smallest = min(N.ravel(a))
...
the list/tuple passed to 'limits'.   Note: either limit in the list/tuple,
or the value of limits itself, can be set to None.  The inclusive sequence
...
1st moment).  Used to calculate coefficients of skewness and kurtosis.
...
"""
    ...
    return amoment(a,4,dimension) - 3


 def adescribe(inarray,dimension=None):
     """
Returns some descriptive statistics of the passed array.  Dimension defaults
to None (i.e., array is unraveled first).
...
Usage:   ascoreatpercentile(inarray,percent)
"""
    if percent > 1:
	print "\nDividing percent>1 by 100 in ascoreatpercentile().\n"
	percent = percent / 100.0
 ...
 def apercentileofscore (inarray, score,histbins=10,defaultlimits=None):
    """
Returns the percentile value of a score relative to the distribution
given by inarray.  Formula depends on the values used to histogram the data(!).
...
of the histogram binning, and (iii) the bin width (the last 2 are not
necessarily integers).  Default number of bins is 10.  If no Lower and
Upper Real Limits are given (as a sequence for the 3rd argument), the
routine picks bins spanning all the numbers in the inarray.  Returns all of
the following:  array of bin values, lowerreallimit, binsize, extrapoints.

Usage:   ahistogram(inarray,numbins,defaultlimits=None,printextras=1)
"""
	...
	estbinwidth = float(max(inarray)-min(inarray))/float(numbins) + 1
	binsize = (max(inarray)-min(inarray)+estbinwidth)/float(numbins)
	lowerreallimit = min(inarray) - binsize/2.0  #lower real limit,1st bin
...
Usage:   aobrientransform(*args)    one 1D array per level of factor
Returns: transformed data for use in an ANOVA
"""
    TINY = 1e-10
    k = len(args)
 ...
 def asamplevar (inarray, dimension=None, keepdim=0):
    """
Returns the standard deviation of the values in the passed array
(i.e., using N).  Defaults to ALL values in the array.  Use dimension=None
to flatten array first.  Set keepdim=1 to return an array with the same
number of dimensions as inarray.

Usage:   asamplevar(inarray,dimension=None,keepdim=0)
"""
    if dimension == None:
	inarray = N.ravel(inarray)
	dimension = 0
    ...
    svar = ass(deviations,dimension,keepdim) / float(inarray.shape[dimension])
    return svar


 def asamplestdev (inarray, dimension=None, keepdim=0):
    """
Returns the sample standard deviation of the values in the passed
array (i.e., using N).  Defaults to ALL values in the array.  Use
dimension=None to flatten array first.  Set keepdim=1 to return an
array with the same number of dimensions as inarray.

Usage:   asamplestdev(inarray,dimension=None,keepdim=0)
"""
    return N.sqrt(asamplevar(inarray,dimension,keepdim))


 def avar (inarray, dimension=None,keepdim=0):
    """
Returns the estimated population variance of the values in the
passed array (i.e., N-1).  Defaults to ALL values in the array.
Use dimension=None to flatten array first.  Set keepdim=1 to return
an array with the same number of dimensions as inarray.

Usage:   avar(inarray,dimension=None,keepdim=0)
"""
    if dimension == None:
	inarray = N.ravel(inarray)
	dimension = 0
    mn = amean(inarray,dimension,1)
    deviations = inarray - mn
    var = ass(deviations,dimension,keepdim)/float(inarray.shape[dimension]-1)
    return var


 def astdev (inarray, dimension=None, keepdim=0):
    """
Returns the estimated population standard deviation of the values
in the passed array (i.e., N-1).  Defaults to ALL values in the array.
Use dimension=None to flatten array first.  Set keepdim=1 to return an
array with the same number of dimensions as inarray.

Usage:   astdev(inarray,dimension=None,keepdim=0)
"""
    return N.sqrt(avar(inarray,dimension,keepdim))


 def asterr (inarray, dimension=None, keepdim=0):
    """
Returns the estimated population standard error of the values
in the passed array (i.e., N-1).  Defaults to ALL values in the array.
Use dimension=None to flatten array first.  Set keepdim=1 to return an
array with the same number of dimensions as inarray.

Usage:   asterr(inarray,dimension=None,keepdim=0)
"""
    if dimension == None:
	inarray = N.ravel(inarray)
	dimension = 0
    return astdev(inarray,dimension,keepdim) / float(N.sqrt(inarray.shape[dimension]))


 def asem (inarray, dimension=None, keepdim=0):
    """
Returns the standard error of the mean (i.e., using N) of the
values in the passed array.  Defaults to ALL values in the array.  Use
dimension=None to flatten array first.  Set keepdim=1 to return an
array with the same number of dimensions as inarray.

Usage:   asem(inarray,dimension=None)
"""
    if dimension == None:
	inarray = N.ravel(inarray)
	dimension = 0
    s = asamplestdev(inarray,dimension,keepdim) / N.sqrt(inarray.shape[dimension]-1)
    return s
 ...
 def az (a, score):
    """
Returns the z-score for a given input score, given that score and the
array from which that score came.  Not appropriate for population
calculations, nor for arrays > 1D.
...
"""
 ...
 def azs (a):
    """
Returns an array of z-scores, one for each score in the passed array.
...
#####################################
 ...
 def atrimboth (a,proportiontocut):
    """
Slices off the passed proportion of items from BOTH ends of the passed
array (i.e., with proportiontocut=0.1, slices 'leftmost' 10% AND 'rightmost'
10% of scores.  You must pre-sort the array so that "proper" trimming is
achieved.  Slices off LESS if proportion results in a non-integer slice
...
"""
    ...
    r_den = math.sqrt((n*ass(x) - asumsquared(x))*(n*ass(y)-asumsquared(y)))
    r = (r_num / r_den)
    df = n-2
    t = r*math.sqrt(df/((1.0-r+TINY)*(1.0+r+TINY)))
    prob = betai(0.5*df,0.5,df/(df+t*t))
...
Usage:   alinregress(x,y)      where x,y are equal length arrays
...
"""
    ...
    t = r*math.sqrt(df/((1.0-r+TINY)*(1.0+r+TINY)))
    prob = betai(0.5*df,0.5,df/(df+t*t))
    slope = r_num / (float(n)*ass(x) - asumsquared(x))
    intercept = ymean - slope*xmean
...
"""
	...
	outputpairedstats(printit,writemode,
			  'Population','--',popmean,0,0,0,
			  name,n,x,v,min(N.ravel(a)),max(N.ravel(a)),
			  statname,t,prob)
    return t,prob
...
scores a, and b.  From Numerical Recipies, p.483.  If printit=1, results
are printed to the screen.  If printit='filename', the results are output
to 'filename' using the given writemode (default=append).
...
"""
    ...
    probs = N.zeros(N.ravel(t).shape,N.Float)
    for i in range(len(N.ravel(t))):
	probs[i] = betai(0.5*df,0.5,float(df)/(df+N.ravel(t)[i]*N.ravel(t)[i]))
	...
	statname = 'Independent samples T-test.'
	outputpairedstats(printit,writemode,
			  name1,n1,x1,v1,min(N.ravel(a)),max(N.ravel(a)),
			  name2,n2,x2,v2,min(N.ravel(b)),max(N.ravel(b)),
			  statname,t,probs)
	return
    return t, probs


 def attest_rel (a,b,printit=0,name1='Samp1',name2='Samp2',writemode='a'):
    """
Calculates the t-obtained T-test on TWO RELATED samples of scores,
a and b.  From Numerical Recipies, p.483.  If printit=1, results are
printed to the screen.  If printit='filename', the results are output to
'filename' using the given writemode (default=append).

Usage:   attest_rel(a,b,printit=0,name1='Samp1',name2='Samp2',writemode='a')
Returns: t-value, two-tailed p-value
"""
    if len(a)<>len(b):
	...
	statname = 'Related samples T-test.'
	outputpairedstats(printit,writemode,
			  name1,n,x1,v1,min(N.ravel(a)),max(N.ravel(a)),
			  name2,n,x2,v2,min(N.ravel(b)),max(N.ravel(b)),
			  statname,t,prob)
	return
    return t, prob
...
Numerical Recipies in C, page 493.  Returns KS D-value, prob.

Usage:   aks_2samp(data1,data2)   data1&2 are arrays of values for 2 conditions
...
Calculates a Mann-Whitney U statistic on the provided scores and
returns the result.  Use only when the n in each condition is < 20 and you
have 2 independent samples of ranks.  REMEMBER: Mann-Whitney U is significant
...
"""
    z = abs(x)
    t = 1.0 / (1.0+0.5*z)
    ans = t * exp(-z*z-1.26551223 + t*(1.00002368+t*(0.37409196+t*(0.9678418+t*(-0.18628806+t*(0.27886807+t*(-1.13520398+t*(1.48851587+t*(-0.82215223+t*0.17087277)))))))))
    if x >= 0:
	return ans
    else:
	return 2.0 - ans
...
given the degrees of freedom for the numerator (dfR-dfF) and the
degrees of freedom for the denominator (dfF).
...
betai.  (Adapted from: Numerical Recipies in C.)
...
lists-of-lists.  (The pstat.get() function can load such a list from a
flat text file.)
"""
...
array of 1s and 0s corresponding to whether or not the subject is a member
of that between-subjects variable combo.  source is the code for the
type of model to calculate.  source=-1 means no restriction; source=0
means to restrict workd's grand mean; source>0  means to restrict the
columns of the main data array, DA, specified (in binary) by the source-value.
...
"""
    ...
    ssbn = 0
    for a in args:
	ssbn = ssbn + asumsquared(N.array(a))/float(len(a))
    ssbn = ssbn - (asumsquared(alldata)/float(bign))
    sswn = sstot-ssbn
    dfbn = na-1
    dfwn = bign - na
...
#####################################
 ...
 def asum (a, dimension=None,keepdim=0):
     """
An alternative to the Numeric.add.reduce function, which allows one to
(1) collapse over multiple dimensions at once, and/or (2) to retain all
dimensions in the original array (squashing one down to size.  If dimension
is set to None, input array is unraveled prior to summing.  If dimension
is a sequence, multiple dims will be collapsed simultaneously.  If keepdim=1,
the resulting array will have as many dimensions as the input array.

Usage:   asum(a, dimension=None, keepdim=0)
Returns: array summed along 'dimension'(s), same _number_ of dims if keepdim=1
"""
     if dimension == None:
	 s = N.sum(N.ravel(a))
     elif type(dimension) in [IntType,FloatType]:
...
passed array.  dimension can be None (a is unraveled first), or an integer.
...
the array.  Use dimension=None to flatten array first, use a sequence to
collapse over multiple dimensions.  Set keepdims=1 to maintain the
original number of dimensions.

Usage:   ass(inarray, dimension=None, keepdims=0)
Returns: sum-along-'dimension'(inarray*inarray)
"""
    if dimension == None:
	inarray = N.ravel(inarray)
	dimension = 0
 ...
 def asummult (array1,array2,dimension=None):
    """
...
If dimension = None, arrays are unraveled first.  A trivial function, but
included for consistency.

Usage:   asummult(array1,array2,dimension=None)
"""
    if dimension == None:
	array1 = N.ravel(array1)
	array2 = N.ravel(array2)
	dimension = 0
    return N.add.reduce(array1*array2,dimension)


 def asumsquared(inarray, dimension=None, keepdims=0):
    """
Adds the values in the passed array, squares that sum, and returns
the result.  Defaults to ALL values in the array.  Use dimension=None
to flatten array first.

Usage:   asumsquared(inarray, dimension=None, keepdims=0)
Returns: the square of the sum over dim(s) in dimension
"""
    if dimension == None:
	inarray = N.ravel(inarray)
	dimension = 0
    sum = asum(inarray,dimension,keepdims)
    return sum*sum


 def asumdiffsquared(a,b):
    """
...
equals the entire data array (i.e., column 1=random factor, last column =
measured values.
